import EuclideanGeometry.Auxiliary
import Mathlib
...
lemma pmidpoint_foot_perp_bisector{a b : Point}(ab : a ≠ b): foot (pmidpoint a b) (perp_bisector ab) = pmidpoint a b := by{
...
theorem circle_around_unique{a b c : Point}{C : CCircle}(h : noncolinear a b c)(hC: Lies_on_circle a C ∧ Lies_on_circle b C ∧ Lies_on_circle c C): C = Circle_around h := by{
  ...
  have : Radius (Circle_through z R) = Radius (Circle_around h) := by{
    exact same_center_point_radius a t hCa ha
  }
...
lemma center_circle_around_lies_on_perp_bisector{a b c : Point}(h : noncolinear a b c): Lies_on (Center (Circle_around h)) (perp_bisector (noncolinear_imp_pairwise_different12 h)) ∧ Lies_on (Center (Circle_around h)) (perp_bisector (noncolinear_imp_pairwise_different23 h)) ∧ Lies_on (Center (Circle_around h)) (perp_bisector (noncolinear_imp_pairwise_different13 h)) := by{
  constructor
